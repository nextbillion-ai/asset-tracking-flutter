...
class TripSummaryScreenState extends State<TripSummaryScreen> with ToastMixin {
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          if (tripSummary!.createdAt != null)
            _buildInfoItem('Created At', timeFormat(tripSummary!.createdAt)),
          if (tripSummary!.updatedAt != null)
            _buildInfoItem('Updated At', timeFormat(tripSummary!.updatedAt)),
          if (tripSummary!.duration != null)
            _buildInfoItem('Duration', '${(tripSummary!.duration! / 1000 / 60).toStringAsFixed(1)} minutes'),
        ],
      ),
    ),
  );
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          if (tripSummary!.distance != null)
            _buildInfoItem('Total Distance', '${(tripSummary!.distance! / 1000).toStringAsFixed(2)} km'),
          if (tripSummary!.geometry != null) ...[
            _buildInfoItem('Geometry Data', '${tripSummary!.geometry!.length} coordinate points'),
            const SizedBox(height: 8),
            _buildExpandableSection('View Geometry Data', tripSummary!.geometry!.join(', ')),
          ],
          if (tripSummary!.stops != null && tripSummary!.stops!.isNotEmpty) ...[
            ...
            const SizedBox(height: 8),
            ...tripSummary!.stops!.asMap().entries.map((MapEntry<int, TripStop> entry) =>
              _buildStopInfo(entry.key + 1, entry.value)
            ),
          ],
          if (tripSummary!.route != null && tripSummary!.route!.isNotEmpty) ...[
            const SizedBox(height: 12),
            Text(
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          _buildInfoItem('Created At', timeFormat(DateTime.fromMillisecondsSinceEpoch((tripSummary!.asset.createdAt * 1000).toInt()))),
        ],
      ),
    ),
  );
    ...
    child: Column(
      ...
      children: <Widget>[
        ...
        if (stop.metaData != null && stop.metaData!.isNotEmpty) ...[
          const SizedBox(height: 4),
          const Text('MetaData:', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
  ...
  String _getRouteStatsText() {
    ...
    final double avgSpeed = route
        ...
        .fold<double>(0, (double sum, double speed) => sum + speed) / route.length;
    ...
    final double avgAccuracy = route
        ...
        .fold<double>(0, (double sum, double acc) => sum + acc) / route.length;
